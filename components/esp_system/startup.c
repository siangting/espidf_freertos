...
static void do_system_init_fn(uint32_t stage_num)
{
    ...
    for (p = &_esp_system_init_fn_array_start; p < &_esp_system_init_fn_array_end; ++p) {
        if (p->stage == stage_num && (p->cores & BIT(core_id)) != 0) {
            ...
            ESP_EARLY_LOGI(TAG, "calling init function: %p on core: %d", p->fn, core_id);

            /* ─────── Profiling Start ─────── */
            int64_t t0 = esp_timer_get_time();
            ESP_EARLY_LOGD("INIT_PROF", "stage %u fn@%p start", stage_num, p->fn);

            esp_err_t err = (*(p->fn))();

            int64_t t1 = esp_timer_get_time();
            ESP_EARLY_LOGI("INIT_PROF", "stage %u fn@%p end, took %lld μs",
                     stage_num, p->fn, (long long)(t1 - t0));
            /* ─────── Profiling End ─────── */
